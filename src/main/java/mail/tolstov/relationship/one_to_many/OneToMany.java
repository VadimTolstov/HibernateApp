...
//            person5.getItems().add(item3);
            ...
            System.out.println("Ленивая загрузка данных ");
            Person person6 = session.get(Person.class, 1);
            Hibernate.initialize(person6.getItems());///получаем данные с ленивой загрузкой, после  .commit() можем использовать данные


            session.getTransaction().commit();
            System.out.println("Используем данные после ленивой загрузки " + person6.getItems());

            System.out.println("Открываем вторую транзакцию для получения данных");
            //получаем новую сессию для открытия новой транзакции
            session = sessionfactory.getCurrentSession();
            session.beginTransaction();
            person6 = (Person) session.merge(person6);
            Hibernate.initialize(person6.getItems());
            session.getTransaction().commit();
